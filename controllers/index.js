       ...
       var fileid = Date.now() + '' + ((Math.random() * 1e6) | 0);
        ...
        return async.series([
          ...
          function(callback) {
            ...
            return request.post('http://10.12.135.37/api/landpageHtml.do?lpageUrl=' + encodeURIComponent(fileurl + "?fl=" + page.lpid + '&gpbtok=1') + '&lpageFl=' + page.lpid + '&lpageName=' + encodeURIComponent(page.lpname), function(error, response, body) {
              if (error) {
