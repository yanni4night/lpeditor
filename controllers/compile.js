...
exports.compile = function(config, debug, callback) {
    ...
    config.sys.flashjs = swig.render(flashjs.toString(), {
        locals: config
    });
    config.sys.flashloadingjs = swig.render(flashloadingjs.toString(), {
        locals: config
    });
    ...
    if (!debug) {
        ...
        config.sys.flashjs = uglify.minify(config.sys.flashjs, {
            fromString: true
        }).code;
        config.sys.flashloadingjs = uglify.minify(config.sys.flashloadingjs, {
            fromString: true
        }).code;

        config.sys.css = new cleancss({
            keepSpecialComments: 0,
            noAdvanced: 1
        }).minify(config.sys.css);
        config.sys.navcss = new cleancss({
            keepSpecialComments: 0,
            noAdvanced: 1
        }).minify(config.sys.navcss);
        config.css = new cleancss({
            keepSpecialComments: 0,
            noAdvanced: 1
        }).minify(config.css);
    }
