(function() {
    ...
    var RegForm = function() {
        ...
        return {
            ...
            submit: function() {
                ...
                if (isloginaction) {
                    LandingPage.login(query.userid, query.psw, function() {
                        Dialog.tip('reg', '用户名或密码错误');
                    });
                    return;
                }
                ...
                utils.ajax({
                    ...
                    onsuccess: function(status) {
                        if (!+status) {
                            ...
                            LandingPage.login(utils.get('input-reg-user').value, utils.get('input-reg-pwd').value, function() {
                                Dialog.tip('reg', '系统繁忙');
                            });
                        } else {
                            utils.pb.cl(utils.merge(utils.clone(STATS_CONFIG), {
    ...
    var LoginForm = function() {
        ...
        return {
            ...
            submit: function() {
                ...
                LandingPage.login(utils.get('input-login-user').value, utils.get('input-login-pwd').value, function() {
                    Dialog.tip('login', '用户名或密码错误');
                });
            }
        };
    }();
    ...
    var LandingPage = {
        ...
        enter: function() {
            ...
            var url = '/preplay.do?gid=' + LP_CONFIG.gid +'&sid=0&source=' + LP_CONFIG.source + _wanclient;
    ...
    PassportSC.on(evts.login_success, function() {
        LandingPage.enter();
    }).on(evts.login_failed, function() {
        ('function' === typeof onfailureFunc) && onfailureFunc();
    }).on(evts.pc_roam_success, function(e, data) {
        //登录态带入检测成功
        //
